...
export default async function DriversPage({
  ...
  const { data: currentUser } = authData?.user
    ? await supabase
        .from("drivers")
        .select("role")
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-slate-600 mt-1">
          {isAdmin
            ? "Approve drivers and manage compensation overrides. Leave % fields blank to use Company Settings defaults."
            : "View drivers for load assignment."}
        </p>
      </div>
      ...
      {isAdmin && rejectedDrivers && rejectedDrivers.length > 0 && (
        <section>
          <h2 className="text-lg font-semibold text-slate-900 mb-4">
